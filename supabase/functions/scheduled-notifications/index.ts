...
serve(async (req) => {
  ...
  // This job runs with the service role key, so it must never be publicly invokable.
  // Only the scheduler (which knows CRON_SECRET) or a service-role caller may run it.
  ...
  const authorized =
    ...
    (!!serviceKey && providedSecret === serviceKey);
