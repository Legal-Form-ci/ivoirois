...
-- ============================================================
-- DEFINITIVE RLS FIX: Convert ALL restrictive policies to PERMISSIVE
-- Plus restore missing triggers and add notification triggers
-- ============================================================
...
CREATE POLICY "cs_insert" ON public.call_signals FOR INSERT TO authenticated WITH CHECK (auth.uid() = caller_id);
CREATE POLICY "cs_delete" ON public.call_signals FOR DELETE TO authenticated USING (auth.uid() = caller_id OR auth.uid() = callee_id);
...
CREATE POLICY "ls_delete" ON public.live_streams FOR DELETE TO authenticated USING (auth.uid() = host_id);
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_roles') THEN
    EXECUTE 'CREATE POLICY "ur_select" ON public.user_roles FOR SELECT TO authenticated USING (true)';
  END IF;
END $$;

-- USER_INTERACTIONS (if exists)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_interactions') THEN
    EXECUTE 'CREATE POLICY "ui_select" ON public.user_interactions FOR SELECT TO authenticated USING (auth.uid() = user_id)';
    EXECUTE 'CREATE POLICY "ui_insert" ON public.user_interactions FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id)';
  END IF;
END $$;
...
BEGIN
  ...
  IF v_post_owner IS NOT NULL AND v_post_owner != NEW.user_id THEN
    INSERT INTO notifications (user_id, from_user_id, post_id, type, content)
    VALUES (v_post_owner, NEW.user_id, NEW.post_id, 'like', 'a aimé votre publication');
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER on_new_like AFTER INSERT ON public.likes FOR EACH ROW EXECUTE FUNCTION public.notify_on_like();
...
BEGIN
  ...
  IF v_post_owner IS NOT NULL AND v_post_owner != NEW.user_id THEN
    INSERT INTO notifications (user_id, from_user_id, post_id, type, content)
    VALUES (v_post_owner, NEW.user_id, NEW.post_id, 'reaction', 'a réagi à votre publication');
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER on_new_reaction AFTER INSERT ON public.reactions FOR EACH ROW EXECUTE FUNCTION public.notify_on_reaction();
...
BEGIN
  ...
  IF v_post_owner IS NOT NULL AND v_post_owner != NEW.user_id THEN
    INSERT INTO notifications (user_id, from_user_id, post_id, type, content)
    VALUES (v_post_owner, NEW.user_id, NEW.post_id, 'comment', 'a commenté votre publication');
  END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.status = 'pending' THEN
    ...
    VALUES (NEW.friend_id, NEW.user_id, 'friend_request', 'vous a envoyé une demande d''ami');
  ELSIF TG_OP = 'UPDATE' AND NEW.status = 'accepted' AND OLD.status = 'pending' THEN
    INSERT INTO notifications (user_id, from_user_id, type, content)
    VALUES (OLD.user_id, NEW.friend_id, 'friend_accepted', 'a accepté votre demande d''ami');
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER on_friendship_change AFTER INSERT OR UPDATE ON public.friendships FOR EACH ROW EXECUTE FUNCTION public.notify_on_friend_request();
